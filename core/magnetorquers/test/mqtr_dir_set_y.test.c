/**
 * @file mqtr_dir_set_y.test.c
 * @author Carl Mattatall (cmattatall2@gmail.com)
 * @brief Source module to test magnetorquer API for y direction
 * @version 0.1
 * @date 2021-02-18
 * 
...
int main(void)
{
    ...
    for (i = 0; i < 2; i++)
    {
        ...
        mqtr_set_dir(MQTR_y, dirs[i]);
        if (dirs[i] != mqtr_get_dir(MQTR_y))
        {
            return -1;
        }
    }
    ...
    {
        if (MQTR_DIR_invalid != mqtr_set_dir(MQTR_y, i))
        {
            return i;
        }
    }
