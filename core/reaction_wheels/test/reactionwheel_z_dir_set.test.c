/**
 * @file reactionwheel_z_dir_set_check.c
 * @author Carl Mattatall (cmattatall2@gmail.com)
 * @brief Source module to test reactionwheel interface API functionality for
 * setting/getting reaction wheel Z direction value
 * @version 0.1
 * @date 2020-12-27
 * 
...
int main(void)
{
    ...
    for (i = 0; i < 2; i++)
    {
        reacwheel_set_wheel_dir(REACTION_WHEEL_z, dirs[i]);
        if (dirs[i] != reacwheel_get_wheel_dir(REACTION_WHEEL_z))
        {
            return -1;
        }
    }
    ...
    {
        if (RW_DIR_invalid != reacwheel_set_wheel_dir(REACTION_WHEEL_z, i))
        {
            return i;
        }
    }
