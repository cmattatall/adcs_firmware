...
int main(void)
{
    ...
    for (i = 0; i < 2; i++)
    {
        set_reaction_wheel_dir(REACTION_WHEEL_z, dirs[i]);
        if (dirs[i] != get_reaction_wheel_dir(REACTION_WHEEL_z))
        {
            return -1;
        }
    }
    ...
    {
        if (RW_DIR_invalid != set_reaction_wheel_dir(REACTION_WHEEL_z, i))
        {
            return i;
        }
    }
