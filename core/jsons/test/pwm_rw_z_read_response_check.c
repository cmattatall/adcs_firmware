/**
 * @file pwm_rw_z_read_response_check.c
 * @author Carl Mattatall (cmattatall2@gmail.com)
 * @brief Source module to test the response to {"pwm_rw_z":"read"} OBC request
 * @version 0.1
 * @date 2020-12-27
 *
...
int main(void)
{
    uint8_t json[] = "{\"pwm_rw_z\" : \"read\" }";

    char  expect[250];
    pwm_t pwm;
    for (pwm = PWM_MIN; pwm < PWM_MAX; pwm++)
    {
        reacwheel_set_wheel_pwm(REACTION_WHEEL_z, pwm);
