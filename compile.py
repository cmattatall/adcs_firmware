...
def configure(sdir=".", bdir="build", btype="Debug", cross_compile=False, build_tests=False, verbose=False, build_examples=False, analyze=False):
...
def main():
    ...
    parser.add_argument("--analyze", action="store_true", default=False, dest="analyze", help="Option to run static analysis after the generation step")
    ...
    if 0 != configure(sdir=args.sdir, bdir=args.bdir,cross_compile=args.cross, btype=args.btype, build_tests=args.make_tests, verbose=args.verbose, build_examples=args.build_examples, analyze=args.analyze):
        if args.verbose:
